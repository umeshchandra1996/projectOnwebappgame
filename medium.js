...
$(document).on('keydown', function(kd)
{
    ...
    function left() 
    {
 	...
 	function right() 
    {
         if (gameover === false && parseInt(car.css('left')) < gameboard_width - car_width) 
         {
            car.css('left', parseInt(car.css('left')) + 5);
            move_right = requestAnimationFrame(right);
        }
 ...
 anim_id = requestAnimationFrame(repeat);
    ...
    function repeat() {
        if (collision(car, car_1) || collision(car, car_2) || collision(car, car_3)  || collision(car,truck)) 
        {   
             audio_c();
            stop_the_game();
            return;
        ...
        if (score_counter % 25 == 0) {
           ...
           /* scorebox.text(parseInt(scorebox.text()) + 1);*/
        }
